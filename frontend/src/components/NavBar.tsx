...
export default function NavBar() {
  ...
  return (
    <nav className="navbar">
      <NavSky theme={dark ? 'dark' : 'light'} />
      <div className="navbar-inner">
        <button className="navbar-brand" onClick={() => scrollTo('about')}>
          <YantraLogo size={24} />
          <span className="navbar-title">YantraGen</span>
        </button>
        ...
        <ul className={`navbar-links${mobileOpen ? ' open' : ''}`}>
          ...
          <li>
            <button
              className={`sound-toggle${soundCtx.muted ? ' muted' : ''}`}
              onClick={toggleMute}
              aria-label={soundCtx.muted ? 'Unmute sound' : 'Mute sound'}
              title={soundCtx.muted ? 'Sound off' : 'Sound on'}
            >
              {soundCtx.muted ? (
